...
def project(
        ...
        *,
        ...
        w_std,
        noise_buffs,
        buf, # for the noise regularization
        device: torch.device,
...
save_visualization= repoConfig.projector.save_visualization
out_vis_dir = repoConfig.projector.out_visualization_dir
steps = repoConfig.projector.steps
w_avg_samples = repoConfig.projector.w_avg_samples
init_lr = repoConfig.projector.init_lr
noise_weight_regularization = repoConfig.projector.noise_weight_regularization #was 1e5
constant_learning_rate = repoConfig.projector.constant_lr # Add flag to use a constant learning rate throughout the optimization (turn off the rampup/rampdown)'
network_pkl = repoConfig.inference.model
target_dir = repoConfig.dir_to_project #r"C:\Users\Rasmu\Repos\StyleGAN\stylegan3-fun-main\out\projection\00025-manual_test\rows"
out_image_dir = repoConfig.projector.out_image_dir 
out_latent_dir = repoConfig.projector.out_latent_dir
out_name_addon = repoConfig.projector.out_name_addon
# ----------------------------------------------------------------------------
...
def run_projection(
    ...
    # \b
    # python projector.py --project-in-wplus --save-video --num-steps=2000 --compress --network="C:\\Users\\Rasmu\\StyleGAN\\stylegan3-main\\training_runs\\00003-stylegan2-GCI_Front_100_formatted-gpus1-batch16-gamma0.8192\\network-snapshot-001200.pkl" --target="C:\\Users\\Rasmu\\StyleGAN\\dataset_stuff\\Grading_Manual_Examples\\BreastBruise5.png"
    # """
    ...
    for target_fname in tqdm(os.listdir(target_fname_directory), desc='Projecting images', unit='image'):
        target_fname_full = os.path.join(target_fname_directory, target_fname)
        target_pil = PIL.Image.open(target_fname_full).convert('RGB')
        w, h = target_pil.size
        s = min(w, h)
        target_pil = target_pil.crop(((w - s) // 2, (h - s) // 2, (w + s) // 2, (h + s) // 2))
        target_pil = target_pil.resize((G.img_resolution, G.img_resolution), PIL.Image.BILINEAR) #Changed to PIL.Image.BILINEAR from PIL.Image.LANCZOS
        ...
        out_img_path = os.path.join(out_image_dir, f"{os.path.splitext(target_fname)[0]+out_name_addon}.png")
        out_latent_path = os.path.join(out_latent_dir, f"{os.path.splitext(target_fname)[0]+out_name_addon}.npy")
        ...
        if save_every_step:
            ...
            for step in tqdm(range(num_steps), desc='Saving projection results', unit='steps'):
                ...
                PIL.Image.fromarray(synth_image, 'RGB').save(f'{target_fname}_step{step:0{n_digits}d}.png')
                np.save(f'{target_fname}_step{step:0{n_digits}d}.npy', w.unsqueeze(0).cpu().numpy())
        else:
            ...
            np.save(out_latent_path, projected_w.unsqueeze(0).cpu().numpy())
            if save_visualization:
                visualization = np.concatenate([np.array(target_pil), synth_image], axis=1)
                out_vis_path = os.path.join(out_vis_dir, f"{os.path.splitext(target_fname)[0]+out_name_addon}.png")
                PIL.Image.fromarray(visualization, 'RGB').save(out_vis_path)

        # Save the optimization video and compress it if so desired
        if save_video:
            out_vid_path = os.path.join(out_vis_dir, f"{os.path.splitext(target_fname)[0]+out_name_addon}.mp4")
            video = imageio.get_writer(out_vid_path, mode='I', fps=fps, codec='libx264', bitrate='16M')
            print(f'Saving optimization progress video "{target_fname}.mp4"')
        ...
        if save_video and compress:
            ...
            gen_utils.compress_video(original_video=f'{target_fname}.mp4',
                                    original_video_name=f'{target_fname.split(os.sep)[-1]}',
                                    outdir=out_image_dir,
                                    ctx=ctx)
